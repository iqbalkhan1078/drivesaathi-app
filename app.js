...
/* ===== DriveSaathi Complete Version 1: marketplace/privacy/lifecycle/subscriber/export ===== */
function dsRole(){return p?.role||u?.user_metadata?.role||"customer"}
function dsSafe(v){return esc(v??"")}
function openDriverMarketplace(){openP("driverMarketplacePanel");loadDriverMarketplace()}
async function loadDriverMarketplace(){
 const box=$("#driverMarketplaceList"), count=$("#driverMarketCount"); if(!box)return;
 box.innerHTML="<p>Searching available verified drivers...</p>";
 let q=sb.from("driver_profiles").select("user_id,first_name,middle_name,last_name,driver_type,licence_classes,experience_years,availability,expected_rate,job_preferences,profile_verification_status,verification_status");
 const {data,error}=await q.order("experience_years",{ascending:false});
 if(error){box.innerHTML=`<p>${dsSafe(error.message)}</p>`;return}
 const state=($("#dmState")?.value||"").toLowerCase(),city=($("#dmCity")?.value||"").toLowerCase(),area=($("#dmArea")?.value||"").toLowerCase(),typ=($("#dmType")?.value||"").toLowerCase();
 const ids=(data||[]).map(x=>x.user_id); let profiles=[];
 if(ids.length){const r=await sb.from("profiles").select("id,city,state,area").in("id",ids);profiles=r.data||[]}
 const pm=new Map(profiles.map(x=>[x.id,x]));
 const rows=(data||[]).filter(d=>{const pr=pm.get(d.user_id)||{};const hay=[pr.state,pr.city,pr.area].join(" ").toLowerCase();return (!state||String(pr.state||"").toLowerCase().includes(state))&&(!city||String(pr.city||"").toLowerCase().includes(city))&&(!area||hay.includes(area))&&(!typ||String(d.driver_type||"").toLowerCase()===typ)&&String(d.availability||"").toLowerCase()!=="unavailable"});
 count.textContent=`${rows.length} driver(s) found`;
 box.innerHTML=rows.map(d=>{const pr=pm.get(d.user_id)||{};return `<div class="item resumeCard"><h3>${dsSafe([d.first_name,d.middle_name,d.last_name].filter(Boolean).join(" ")||"Verified Driver")}</h3><p>📍 ${dsSafe([pr.area,pr.city,pr.state].filter(Boolean).join(", ")||"Location not specified")}</p><p><b>Type:</b> ${dsSafe(d.driver_type)} · <b>Experience:</b> ${dsSafe(d.experience_years)} years</p><p><b>Licence/Skills:</b> ${dsSafe(d.licence_classes||"Not specified")}</p><p><b>Availability:</b> ${dsSafe(d.availability)} · <b>Expected:</b> ${dsSafe(d.expected_rate||"Discuss via DriveSaathi")}</p><p><b>Preferences:</b> ${dsSafe((d.job_preferences||[]).join(", "))}</p><div class="privacyBox">🔒 Contact number, email and exact address are private.</div><button class="primary" onclick="requestDriverConnection('${d.user_id}')">Request / Shortlist via DriveSaathi</button></div>`}).join("")||"<p>No matching available drivers found.</p>";
}
async function requestDriverConnection(driverId){
 const {error}=await sb.from("connection_requests").insert({customer_id:u.id,driver_id:driverId,status:"pending"});
 alert(error?(error.code==="23505"?"You already requested this driver.":error.message):"Driver shortlisted. DriveSaathi Admin will manage the connection securely.");
}
function openJobMarketplace(){openP("jobsPanel");if(!$("#jobSearchBar")){$("#jobsPanel h2").insertAdjacentHTML("afterend",`<div id="jobSearchBar" class="marketFilters"><input id="jmState" placeholder="State"><input id="jmCity" placeholder="City"><input id="jmArea" placeholder="Area / Location"><select id="jmType"><option value="">All Job Types</option><option>Permanent</option><option>Temporary / Hourly</option><option>Day-wise</option><option>Trip-wise / Outstation</option><option>Commercial</option></select><button class="primary" onclick="loadJobs()">Search Jobs</button></div><div id="jobMarketCount" class="marketCount"></div>`)}loadJobs()}
const dsOriginalLoadJobs=loadJobs;
loadJobs=async function(){
 const box=$("#jobsList");if(!box)return;box.innerHTML="<p>Loading available jobs...</p>";
 const {data,error}=await sb.from("jobs").select("*").order("created_at",{ascending:false});
 if(error){box.innerHTML=`<p>${dsSafe(error.message)}</p>`;return}
 const {data:mine}=await sb.from("job_applications").select("job_id").eq("driver_id",u.id);const applied=new Set((mine||[]).map(x=>x.job_id));
 const state=($("#jmState")?.value||"").toLowerCase(),city=($("#jmCity")?.value||"").toLowerCase(),area=($("#jmArea")?.value||"").toLowerCase(),typ=($("#jmType")?.value||"").toLowerCase();
 const rows=(data||[]).filter(j=>{if(["closed","fulfilled","cancelled"].includes(String(j.status||"").toLowerCase()))return false;const loc=[j.state,j.city,j.area,j.location].join(" ").toLowerCase();return(!state||String(j.state||loc).toLowerCase().includes(state))&&(!city||String(j.city||loc).toLowerCase().includes(city))&&(!area||loc.includes(area))&&(!typ||String(j.job_type||j.title||"").toLowerCase().includes(typ))});
 if($("#jobMarketCount"))$("#jobMarketCount").textContent=`${rows.length} available job(s) found`;
 box.innerHTML=rows.map(j=>`<div class="item jobCard"><b>${dsSafe(j.title||"Driver Requirement")}</b><p>📍 ${dsSafe([j.area,j.city,j.state].filter(Boolean).join(", ")||j.location||"")}</p><p><b>Type:</b> ${dsSafe(j.job_type||"Driver Requirement")} · <b>Rate:</b> ${dsSafe(j.salary_rate||"To be decided")}</p><p>${dsSafe(j.details||"")}</p><div class="privacyBox">🔒 Customer phone/email/exact address hidden. Connection is managed by DriveSaathi.</div>${applied.has(j.id)?'<p class="okText">✓ Already Applied</p>':`<button class="primary" onclick="applyToJob('${j.id}')">Apply for Job</button>`}</div>`).join("")||"<p>No matching active jobs available.</p>";
}
function openCustomerHistory(){openP("customerHistoryPanel");loadCustomerHistory()}
async function loadCustomerHistory(){const box=$("#customerHistoryList");let r=await sb.from("bookings").select("*").eq("customer_id",u.id).order("created_at",{ascending:false});if(r.error){box.innerHTML=dsSafe(r.error.message);return}box.innerHTML=(r.data||[]).map(x=>`<div class="item"><b>${dsSafe(x.service_type)} · #${String(x.id).slice(0,8)}</b><p>📍 ${dsSafe(x.requirement_location)}</p><p>Status: <b>${dsSafe(x.status||"pending")}</b></p><small>Created: ${new Date(x.created_at).toLocaleString()}</small></div>`).join("")||"<p>No requirement history.</p>"}
async function subscribePlan(planId){
 const {data:plan,error}=await sb.from("subscription_plans").select("*").eq("id",planId).single();if(error){alert(error.message);return}
 const {error:e}=await sb.from("user_subscriptions").insert({user_id:u.id,user_role:dsRole(),plan_id:plan.id,amount:plan.price,status:Number(plan.price)>0?"payment_pending":"active",starts_at:new Date().toISOString(),expires_at:new Date(Date.now()+Number(plan.validity_days)*86400000).toISOString()});
 alert(e?e.message:(Number(plan.price)>0?"Plan selected. Payment is pending; no charge has been made yet.":"Free plan activated."));
 if(!e)loadMyPlans();
}
loadMyPlans=async function(){const role=dsRole();const box=$("#myPlansList");const [pr,sr]=await Promise.all([sb.from("subscription_plans").select("*").eq("audience",role).eq("is_active",true).order("validity_days"),sb.from("user_subscriptions").select("*,subscription_plans(name)").eq("user_id",u.id).order("created_at",{ascending:false}).limit(1)]);if(pr.error){box.innerHTML=`<p>${dsSafe(pr.error.message)}</p>`;return}const cur=sr.data?.[0];box.innerHTML=(cur?`<div class="currentPlan"><b>Current Plan:</b> ${dsSafe(cur.subscription_plans?.name||"Plan")} · ${dsSafe(cur.status)}${cur.expires_at?` · until ${new Date(cur.expires_at).toLocaleDateString()}`:""}</div>`:"")+(pr.data||[]).map(x=>`<div class="item"><b>⭐ ${dsSafe(x.name)}</b><p>₹${Number(x.price||0).toFixed(0)} · ${x.validity_days} days</p><button class="primary" onclick="subscribePlan('${x.id}')">${Number(x.price)>0?"Choose Plan":"Activate Free Plan"}</button></div>`).join("")}
async function loadAdminSubscribers(){const box=$("#adminSubscribersList");let r=await sb.from("user_subscriptions").select("*,subscription_plans(name,audience)").order("created_at",{ascending:false});if(r.error){box.innerHTML=dsSafe(r.error.message);return}box.innerHTML=(r.data||[]).map(x=>`<div class="item"><b>${dsSafe(x.subscription_plans?.audience)} · ${dsSafe(x.subscription_plans?.name)}</b><p>User: ${dsSafe(x.user_id)}</p><p>₹${Number(x.amount||0)} · Status: <b>${dsSafe(x.status)}</b></p><p>${x.starts_at?new Date(x.starts_at).toLocaleDateString():""} → ${x.expires_at?new Date(x.expires_at).toLocaleDateString():""}</p><div class="actions"><button onclick="adminSetSubscription('${x.id}','active')">Activate</button><button onclick="adminSetSubscription('${x.id}','suspended')">Suspend</button><button onclick="adminSetSubscription('${x.id}','cancelled')">Cancel</button></div></div>`).join("")||"<p>No subscription records yet.</p>"}
async function adminSetSubscription(id,status){let r=await sb.from("user_subscriptions").update({status}).eq("id",id);alert(r.error?r.error.message:"Subscription updated.");if(!r.error)loadAdminSubscribers()}
function dsCSV(rows){if(!rows.length)return "";const keys=[...new Set(rows.flatMap(Object.keys))].filter(k=>!["password","aadhaar","pan"].includes(k));const q=v=>`"${String(v??"").replaceAll('"','""')}"`;return [keys.map(q).join(","),...rows.map(r=>keys.map(k=>q(typeof r[k]==="object"?JSON.stringify(r[k]):r[k])).join(","))].join("\n")}
async function exportAdminCSV(){if(dsRole()!=="admin"){alert("Super Admin only.");return}const type=$("#exportType").value,city=($("#exportCity").value||"").trim();let table=type==="drivers"?"driver_profiles":type==="customers"?"profiles":type==="applications"?"job_applications":type==="subscriptions"?"user_subscriptions":"jobs";let q=sb.from(table).select("*");if(type==="customers")q=q.eq("role","customer");if(city&&["profiles","jobs"].includes(table))q=q.ilike("city",`%${city}%`);let r=await q;if(r.error){$("#exportStatus").textContent=r.error.message;return}const blob=new Blob([dsCSV(r.data||[])],{type:"text/csv;charset=utf-8"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=`DriveSaathi_${type}_${new Date().toISOString().slice(0,10)}.csv`;a.click();URL.revokeObjectURL(a.href);$("#exportStatus").textContent=`Exported ${(r.data||[]).length} record(s).`}
